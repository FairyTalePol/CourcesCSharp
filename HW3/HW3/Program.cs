    ...
    class Program
    {
        ...
        public static void Task3()
        {
            ...
            if (int.TryParse(input, out int intNumber))
            {
                ...
                Console.WriteLine($"SqrtBinary({intNumber}) = {SqrtBinary(intNumber)}");
        ...
        static int SqrtBinary(int number)
        {
            int low = 0;
            int high = number + 1;
            while (high - low > 1)
            ...
            return Math.Abs(number - (low + 1) * (low + 1)) < Math.Abs(number - (low*low)) ? (low +1): low;
